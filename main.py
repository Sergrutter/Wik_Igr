...
def search():
    if request.method == 'POST':
        ...
        # Формируем регулярное выражение для нечеткого поиска
        search_pattern = f'%{search_query}%'
        # Используем оператор like для поиска с учетом регистра
        pages = Page.query.filter(func.lower(Page.title).like(func.lower(search_pattern))).all()
